...
async function commitScaffolds(
  ...
  execFileSync('git', ['add', ...filePaths], { stdio: 'inherit' });
  ...
  try {
    ...
    if (shouldCommit) {
      try {
        ...
        // Fallback to comment if commit/push fails, but do not fail the workflow
        core.warning(
          `Failed to commit generated test scaffolds, falling back to PR comment: ${error}`
        );
