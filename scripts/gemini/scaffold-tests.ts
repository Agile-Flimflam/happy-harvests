...
async function commitScaffolds(
  ...
  try {
    ...
    const lastCommitAuthor = execFileSync('git', ['log', '-1', '--pretty=format:%an <%ae>'], {
      encoding: 'utf-8',
      env: SAFE_ENV,
    }).trim();
    // Safe usage: command and arguments are constant strings; no user-controlled data is passed here.
    const lastCommitMessage = execFileSync('git', ['log', '-1', '--pretty=format:%s'], {
      encoding: 'utf-8',
      env: SAFE_ENV,
    }).trim();

    const actionAuthor = 'github-actions[bot] <github-actions[bot]@users.noreply.github.com>';
    const commitSubjectPrefix = 'test: add generated test scaffolds for ';

    if (lastCommitAuthor === actionAuthor && lastCommitMessage.startsWith(commitSubjectPrefix)) {
  ...
  execFileSync('git', ['add', '--', ...filePaths], { stdio: 'inherit', env: SAFE_ENV });
...
Files: ${scaffolds.map((scaffold) => scaffold.filePath.replace(/[`$]/g, '')).join(', ')}`;
    ...
    if (!targetBranch) {
      ...
      const prNumber =
        prNumberEnv && !Number.isNaN(Number.parseInt(prNumberEnv, 10))
          ? Number.parseInt(prNumberEnv, 10)
          : prContext.prNumber;
      ...
      if (headRef && headRepoFullName === baseRepoFullName && isLikelyValidBranchRef(headRef)) {
        targetBranch = sanitizeGitRef(headRef);
      } else {
        core.info(
