...
const SAFE_ENV: NodeJS.ProcessEnv = { ...process.env, PATH: SAFE_PATH };
...
async function commitScaffolds(
  ...
  try {
    ...
    const actionAuthor = 'GitHub Action <action@github.com>';
  ...
  // Stage files
  const filePaths: string[] = scaffolds.map((scaffold) => scaffold.filePath);
...
Files: ${scaffolds.map((scaffold) => scaffold.filePath).join(', ')}`;
