...
export async function getNurseries(): Promise<{ nurseries?: Nursery[]; error?: string }> {
  ...
  const { data, error } = await supabase.from('nurseries').select('*').order('name', { ascending: true });
  if (error) return { error: error.message };
  return { nurseries: (data as Nursery[]) || [] };
}

export async function getLocationsForSelect(): Promise<{ locations?: Pick<Location, 'id' | 'name'>[]; error?: string }> {
  const supabase = await createSupabaseServerClient();
...
export async function createNursery(input: { name: string; location_id: string; notes?: string }) {
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.from('nurseries').insert({ name: input.name, location_id: input.location_id, notes: input.notes ?? null });
  if (error) return { error: error.message };
  return { ok: true } as const;
}

export async function updateNursery(input: { id: string; name: string; location_id: string; notes?: string }) {
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.from('nurseries').update({ name: input.name, location_id: input.location_id, notes: input.notes ?? null }).eq('id', input.id);
  if (error) return { error: error.message };
  return { ok: true } as const;
}
...
export async function actionCreateNursery(prev: NurseryFormState, formData: FormData): Promise<NurseryFormState> {
  const supabase = await createSupabaseServerClient();
...
export async function actionUpdateNursery(prev: NurseryFormState, formData: FormData): Promise<NurseryFormState> {
  const supabase = await createSupabaseServerClient();
