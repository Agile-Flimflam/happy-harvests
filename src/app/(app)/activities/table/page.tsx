...
export default async function ActivitiesTablePage({ searchParams }: { searchParams?: { type?: 'irrigation' | 'soil_amendment' | 'pest_management' | 'asset_maintenance'; from?: string; to?: string; location_id?: string; sort?: 'started_at' | 'labor_hours' | 'cost'; dir?: 'asc' | 'desc' } }) {
  const supabase = await createSupabaseServerClient()
  const { data: locations } = await supabase.from('locations').select('id,name').order('name', { ascending: true })
  const { rows, error } = await getActivitiesFlat(searchParams)
    ...
    <div>
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <Button asChild>
            <Link href={{ pathname: '/api/activities/export', query: searchParams as any }}>Export CSV</Link>
          </Button>
        </div>
        <form className="grid gap-3 md:grid-cols-5">
          ...
          <div className="space-y-1">
            ...
            <div className="flex gap-2">
              <select name="sort" defaultValue={searchParams?.sort ?? 'started_at'} className="border rounded px-2 py-1 w-full">
                <option value="started_at">Date</option>
                <option value="labor_hours">Hours</option>
                <option value="cost">Cost</option>
              </select>
              <select name="dir" defaultValue={searchParams?.dir ?? 'desc'} className="border rounded px-2 py-1 w-full">
