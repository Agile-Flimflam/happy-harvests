import Link from 'next/link';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { CalendarPlus, Sprout } from 'lucide-react';
import CalendarClient from './CalendarClient';
import { getCalendarEvents, getCalendarLocations } from './_actions';
import { getLocationsWithWeather } from '../locations/actions';
export const revalidate = 0;
export const dynamic = 'force-dynamic';

export default async function CalendarPage() {
  try {
    const [eventsRes, locationsRes, weatherRes] = await Promise.all([
      getCalendarEvents(),
      getCalendarLocations(),
      getLocationsWithWeather(),
    ]);

    if (!eventsRes.ok) {
      return <div className="text-red-500">Error loading calendar events: {eventsRes.message}</div>;
    }
    if (!locationsRes.ok) {
      return (
        <div className="text-red-500">Error loading calendar locations: {locationsRes.message}</div>
      );
    }
    if (!weatherRes.ok) {
      return <div className="text-red-500">Error loading weather data: {weatherRes.message}</div>;
    }

    const weatherByLocation = weatherRes.data.weatherByLocation;
    return (
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <h1 className="text-3xl font-bold">Calendar</h1>
          <div className="grid grid-cols-1 sm:flex gap-2 w-full sm:w-auto">
            <Button asChild>
              <Link href="/activities/new" aria-label="Schedule Activity">
                <CalendarPlus className="mr-2 h-4 w-4" /> Schedule Activity
              </Link>
            </Button>
            <Button asChild variant="outline">
              <Link href="/plantings" aria-label="Manage Plantings">
                <Sprout className="mr-2 h-4 w-4" /> Manage Plantings
              </Link>
            </Button>
          </div>
        </div>
        <Card>
          <CardContent className="p-0">
            <div className="rounded-xl bg-gradient-to-b from-muted/20 via-muted/10 to-transparent p-3 sm:p-4">
              <CalendarClient
                events={eventsRes.data.events}
                locations={locationsRes.data.locations}
                weatherByLocation={weatherByLocation}
              />
            </div>
          </CardContent>
        </Card>
      </div>
    );
  } catch (error) {
    console.error('[Calendar] Failed to load calendar page data', error);
    return <div className="text-red-500">Error loading calendar data. Please try again.</div>;
  }
}
