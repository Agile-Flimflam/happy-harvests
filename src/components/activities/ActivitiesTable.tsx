...
export function ActivitiesTable({ rows, bulkDeleteAction }: ActivitiesTableProps) {
  ...
  const columns = React.useMemo<ColumnDef<Row>[]>(() => [
    ...
    },
    {
      ...
      header: ({ column }) => (
        <SortHeader column={column} title="Date" align="left" />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <SortHeader column={column} title="Hours" align="right" />
      ),
    ...
    },
    {
      ...
      header: () => null,
      cell: ({ row }) => (
        <Button asChild size="sm" variant="outline"><Link href={`/activities/${row.original.id}/edit`}>Edit</Link></Button>
      ),
      enableSorting: false,
    },
      ...
      <table className="w-full text-sm">
        <thead className="bg-accent/40">
          {table.getHeaderGroups().map((hg) => (
            <tr key={hg.id}>
              {hg.headers.map((h) => (
                <th key={h.id} className={`p-2 ${h.column.id === 'labor_hours' || h.column.id === 'cost' ? 'text-right' : 'text-left'}`} aria-sort={ariaSort(h.column.getIsSorted())}>
          ...
          {table.getRowModel().rows.map((r) => (
            <tr key={r.id} className="border-t">
              {r.getVisibleCells().map((c) => (
                <td key={c.id} className={`p-2 ${c.column.id === 'labor_hours' || c.column.id === 'cost' ? 'text-right' : ''}`}>
...
function SortHeader<RowData>({ column, title, align = 'left' }: { column: Column<RowData, unknown>; title: string; align?: 'left' | 'right' }) {
