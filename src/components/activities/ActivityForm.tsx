...
export function ActivityForm({
  ...
  const initialState: ActivityFormState = { success: false, message: '', errors: {} };
  ...
  const [templates, setTemplates] = React.useState<
    Array<{ name: string; payload: TemplatePayload }>
  >([]);
  ...
  function saveTemplate() {
    ...
    setTemplates(next);
    try {
      localStorage.setItem('hh_activity_templates', JSON.stringify(next));
    } catch {}
  }
  ...
  return (
    <form action={formAction} className="grid gap-4 md:grid-cols-2">
      ...
      <div className="col-span-full text-xs font-semibold uppercase text-muted-foreground">
        Step 1 · What
      </div>
      <div className="space-y-2">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => setStartValue(formatNowLocal())}
              >
                Now
              </Button>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <Popover>
                ...
                <PopoverContent className="w-auto overflow-hidden p-0" align="start">
                  <Calendar
                    ...
                    onSelect={(d) =>
                      setStartValue(
                        combine(formatDateOnly(d || undefined), getTimePart(startValue))
                      )
                    }
                  />
                </PopoverContent>
              ...
              <Input
                ...
                onChange={(e) =>
                  setStartValue(combine(getDatePart(startValue), e.currentTarget.value))
                }
                required
                step="1"
            ...
            {startValue ? (
              <div className="text-xs text-muted-foreground">
                Hawaiian moon:{' '}
                <span className="font-medium">{hawaiianMoonForISO(startValue) ?? '—'}</span>
              </div>
            ) : null}
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => setEndValue(formatNowLocal())}
              >
                Now
              </Button>
            </div>
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto overflow-hidden p-0" align="start">
                  <Calendar
                    ...
                    captionLayout="dropdown"
                    onSelect={(d) =>
                      setEndValue(combine(formatDateOnly(d || undefined), getTimePart(endValue)))
                    }
                  />
                </PopoverContent>
              </Popover>
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => setEndFromStartDelta(15)}
              >
                +15m
              </Button>
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => setEndFromStartDelta(30)}
              >
                +30m
              </Button>
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => setEndFromStartDelta(60)}
              >
                +1h
              </Button>
              <Button type="button" size="sm" variant="ghost" onClick={() => setEndValue('')}>
                Clear
              </Button>
            </div>
            {endValue ? (
              <div className="text-xs text-muted-foreground">
                Hawaiian moon:{' '}
                <span className="font-medium">{hawaiianMoonForISO(endValue) ?? '—'}</span>
              </div>
            ) : null}
          </div>
        </>
        ...
        <div className="col-span-full">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <div className="space-y-1 md:col-span-1">
              ...
              >
                <SelectTrigger
                  className={`w-full ${!locationId ? 'pointer-events-none opacity-50' : ''}`}
                ...
                <SelectContent>
                  ...
                  {plots
                    ...
                    .map((p) => (
                      <SelectItem key={p.plot_id} value={String(p.plot_id)}>
                        {p.name}
                      </SelectItem>
                    ))}
                </SelectContent>
            ...
            <div className="space-y-1 md:col-span-1">
              ...
              <Select
                value={bedId || '__none__'}
                onValueChange={(v) => setBedId(v === '__none__' ? '' : v)}
              >
                <SelectTrigger
                  className={`w-full ${!locationId ? 'pointer-events-none opacity-50' : ''}`}
                  aria-disabled={!locationId}
                ...
                <SelectContent>
                  <SelectItem value="__none__">—</SelectItem>
                  {bedsLocal
                    .filter((b) => (plotId ? String(b.plot_id) === plotId : true))
                    .map((b) => (
                      <SelectItem key={b.id} value={String(b.id)}>
                        {b.name ? b.name : `Bed #${b.id}`}
                      </SelectItem>
                    ))}
                </SelectContent>
              </Select>
              {bedId ? (
                <div className="flex items-center gap-2 text-xs mt-1">
                  <input
                    id="rename_bed_name"
                    placeholder="Rename bed…"
                    className="h-7 border rounded px-2 flex-1"
                  />
                  <Button
                    type="button"
                    size="sm"
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        nameEl!.value = '';
                        setBedsLocal((prev) =>
                          prev.map((b) => (String(b.id) === String(bedId) ? { ...b, name } : b))
                        );
            ...
            <div className="space-y-1 md:col-span-1">
              ...
              <Select
                value={nurseryId || '__none__'}
                onValueChange={(v) => setNurseryId(v === '__none__' ? '' : v)}
              >
                <SelectTrigger
                  className={`w-full ${!locationId ? 'pointer-events-none opacity-50' : ''}`}
                  aria-disabled={!locationId}
            ...
            <div className="flex items-center gap-3">
              ...
              <label className="flex items-center gap-2 text-xs text-muted-foreground">
                <input
                  ...
                  onChange={(e) => setAutoDuration(e.currentTarget.checked)}
                />
                Auto-calc
              </label>
            </div>
      ...
      {visibility.showAmendments ? <AmendmentsEditor /> : null}
      ...
      <div className="col-span-full">
        <div className="flex items-center gap-2">
          <Button type="submit" disabled={!activityType}>
            Save Activity
          </Button>
          <Button type="button" variant="outline" onClick={saveTemplate} disabled={!activityType}>
            Save as Template
          </Button>
          {templates.length ? (
            <select
              ...
              onChange={(e) => {
                if (e.currentTarget.value) loadTemplate(e.currentTarget.value);
              }}
              defaultValue=""
            >
      ...
      {state?.message ? (
        <div className="col-span-full text-sm text-muted-foreground">{state.message}</div>
      ) : null}
    </form>
  );
}
