-- Rename planting_events.quantity to qty and update dependent function/constraint
begin;
...
begin
  if exists (
    ...
    where table_schema = 'public' and table_name = 'planting_events' and column_name = 'quantity'
  ) then
    alter table public.planting_events rename column quantity to qty;
  end if;
end $$;

-- Update constraint to reference qty
...
-- Update initial create functions to insert qty instead of quantity
